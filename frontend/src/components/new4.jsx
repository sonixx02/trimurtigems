...
const DiamondModel = ({ modelPath, materialProps, showWireframe = false }) => {
  ...
  useEffect(() => {
    if (originalScene) {
      ...
      if (selectedModel) {
        ...
        // Create the diamond material with proper settings
        ...
        solidModelClone.material = diamondMaterial;
  ...
  }, [originalScene, shape, materialProps, showWireframe]);
...
const AutoRotate = ({ enabled, rotationSpeed = { x: 0.01, y: 0.01, z: 0.01 } }) => {
  const { scene } = useThree();
